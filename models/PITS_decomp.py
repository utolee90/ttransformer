...
class Model(nn.Module):
    def __init__(self, configs, 
        ...
        self.model_trend = PITS_backbone(enc_in=enc_in, 
        ...
        self.model_res = PITS_backbone(enc_in=enc_in, 
                                context_window = context_window, target_window=target_window, patch_len=patch_len, stride=stride, 
                                d_model=d_model,
                                shared_embedding=shared_embedding,
                                head_dropout=head_dropout, 
    ...
    def forward(self, x):           # x: [Batch, Input length, Channel]
        ...
        res_init, trend_init = self.decomposition(x)
        res_init, trend_init = res_init.permute(0,2,1), trend_init.permute(0,2,1)  # x: [Batch, Channel, Input length]
        res = self.model_res(res_init)
        trend = self.model_trend(trend_init)
        x = res + trend
        return x.permute(0,2,1), trend.permute(0,2,1), res.permute(0,2,1)    # x: [Batch, Input length, Channel]
