...
exchange_96_96_new = "long_term_forecast_COMPARE_iTransformer_Exchange_96_96_Mod-iTransformer_data-exchange_rate.csv_(96to96)_0(1729846858)"
...
a_init , b_init = sigmoid_inverse(0.001), sigmoid_inverse(0.001)  # 초기값(sigmoid로변환할  것 감안)  
# a_init , b_init = 0.01, 0.01
...
for idx in range(1):
    ...
    class CombinedModel(nn.Module):
        ...
        def get_result(self):
            a, b = torch.sigmoid(self.a).detach().cpu().numpy()[0], torch.sigmoid(self.b).detach().cpu().numpy()[0]
            return a,b
    ...
    preds_te_te = [] # 예측값
    trues_te_te = [] # 참값
    preds_te_lin_te = [] # 96_lin
    preds_te_lin_te_48 = [] # 48_lin
    preds_te_lin_te_24 = [] # 24_lin
    preds_te_lin_te_12 = [] # 12_lin
    preds_te_te_x = []

    # 실제 데이터 셋 호출
    result_list = ['pred.npy', 'true.npy']
    result_path = './results/'
    np_pred_new = np.load(f"{result_path}{setting_path}/{result_list[0]}")
    np_true_new = np.load(f"{result_path}{setting_path}/{result_list[1]}")
    ...
    for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(dataset_input_test_loader):
        ...
        with torch.no_grad():
            ...
            outputs_x = res_iTransformer(batch_x)
            ...
            outputs_x = outputs_x[:, -args.pred_len:, :].detach().cpu().numpy()
            batch_y = batch_y.detach().cpu().numpy()
        
            pred = outputs
            true = batch_y

            batch_x_shapes.append(batch_x.shape)

            preds_te_te.append(pred)
            trues_te_te.append(true)
            preds_te_lin_te.append(lin_result)
            preds_te_lin_te_24.append(lin_result_24)
            preds_te_lin_te_48.append(lin_result_48)
            preds_te_lin_te_12.append(lin_result_12)
            preds_te_te_x.append(outputs_x)
    ...
    preds_te_lin_te = np.transpose(torch.concat(preds_te_lin_te, axis=0).detach().cpu().numpy(), (0,2,1))
    preds_te_lin_te_24 = np.transpose(torch.concat(preds_te_lin_te_24, axis=0).detach().cpu().numpy(), (0,2,1))
    preds_te_lin_te_48 = np.transpose(torch.concat(preds_te_lin_te_48, axis=0).detach().cpu().numpy(), (0,2,1))
    preds_te_lin_te_12 = np.transpose(torch.concat(preds_te_lin_te_12, axis=0).detach().cpu().numpy(), (0,2,1))
    preds_te_te_x = np.transpose(np.concatenate(preds_te_te_x, axis=0), (0,2,1))
    input_len_te = i + 1
    print(input_len_te)
    print(batch_x_shapes)
    print()

    data_col_time = time.time()
    print(f"경과 시간: {data_col_time-start_time}")
    print("METRIC 비교")
    print("PRED", MSE(np_pred_new, preds_te_te))
    print("PRED_TE_COMP", MSE(preds_te_te, preds_te_te_x))
    print("TRUE", MSE(np_true_new, trues_te_te))
    np_pred = preds_te_te.copy()
    np_true = trues_te_te.copy()
    
    # 모델 실험
    combine_model_test = CombinedModel(QMAP_FN[q1], QMAP_FN[q2])
    ...
    optimizer = torch.optim.Adam([combine_model_test.a,combine_model_test.b], lr=lr, weight_decay=3e-4)
    ...
    if q1 == "lin96":
        np_pred_first_new = get_np_pred_lin(96)
        np_pred_first = preds_te_lin_te.copy()
        print("PRED_96_DONE")
    elif q1 == "lin48":
        np_pred_first_new = get_np_pred_lin(48)
        np_pred_first = preds_te_lin_te_48.copy()
        print("PRED_48_DONE")
    elif q1 == "lin24":
        np_pred_first_new = get_np_pred_lin(24)
        np_pred_first = preds_te_lin_te_24.copy()
        print("PRED_24_DONE")
    elif q1 == "lin12":
        np_pred_first_new = get_np_pred_lin(12)
        np_pred_first = preds_te_lin_te_12.copy()
        print("PRED_12_DONE")
    
    if q2 == "lin96":
        np_pred_second_new = get_np_pred_lin(96)
        np_pred_second = preds_te_lin_te.copy()
        print("PRED_96_DONE")
    elif q2 == "lin48":
        np_pred_second_new = get_np_pred_lin(48)
        np_pred_second = preds_te_lin_te_48.copy()
        print("PRED_48_DONE")
    elif q2 == "lin24":
        np_pred_second_new = get_np_pred_lin(24)
        np_pred_second = preds_te_lin_te_24.copy()
        print("PRED_24_DONE")
    elif q2 == "lin12":
        np_pred_second_new = get_np_pred_lin(12)
        np_pred_second = preds_te_lin_te_12.copy()
        print("PRED_12_DONE")
    elif q2 == "none":
        np_pred_second_new = np.zeros(np_pred.shape)
    ...
    for j, (a,b) in enumerate(loss_points):
        res_temp = (1-a-b)*np_pred + a*np_pred_first + b*np_pred_second
    ...
    # 마지막으로 비교
    final_res = (1-a-b)*np_pred + a* np_pred_first + b*np_pred_second
    ...
    with open(f'run_ensenble_txt_metric_comparison_{setting_path}_{q1}_{q2}_{time.time()}.txt', 'w', encoding='utf8') as B:
        wr = "COMPARISON_METRIC\n\n"
        # pred vs pred_new
        wr += "OLD_PRED vs NEW_PRED"
        wr += f"MSE of PRED/TRUE : {MSE(np_pred, np_true)} vs {MSE(np_pred_new, np_true_new)}\n"
        wr += f"MAE of PRED/TRUE : {MAE(np_pred, np_true)} vs {MAE(np_pred_new, np_true_new)}\n"
        wr += f"MSE of PRED_FIRST/TRUE : {MSE(np_pred_first, np_true)} vs {MSE(np_pred_first_new, np_true_new)}\n"
        wr += f"MAE of PRED_FIRST/TRUE : {MSE(np_pred_first, np_true)} vs {MSE(np_pred_first_new, np_true_new)}\n"
        wr += f"MSE of PRED_SECOND/TRUE : {MSE(np_pred_second, np_true)} vs {MSE(np_pred_second_new, np_true_new)}\n"
        wr += f"MAE of PRED_SECOND/TRUE : {MSE(np_pred_second, np_true)} vs {MSE(np_pred_second_new, np_true_new)}\n"
        wr += "BETWEEN NEW_METRIC\n"
        wr += f"MSE between TRUE : {MSE(np_true, np_true_new)}\n"
        wr += f"MAE between TRUE : {MAE(np_true, np_true_new)}\n"
        wr += f"MSE between PRED : {MSE(np_pred, np_pred_new)}\n"
        wr += f"MAE between PRED : {MAE(np_pred, np_pred_new)}\n"
        wr += f"MSE between FIRST : {MSE(np_pred_first, np_pred_first_new)}\n"
        wr += f"MAE between FIRST: {MSE(np_pred_first, np_pred_first_new)}\n"
        wr += f"MSE between SECOND : {MSE(np_pred_second, np_pred_second_new)}\n"
        wr += f"MAE between SECOND : {MSE(np_pred_second, np_pred_second_new)}\n"
        wr += f"MSE between PRED PATTERN : {MSE(preds_te_te, preds_te_te_x)}\n"
        wr += f"MAE between PRED PATTERN : {MAE(preds_te_te, preds_te_te_x)}\n"
        B.write(wr)
